...
public class MainActivity extends AppCompatActivity {
    ...
    private void predict(){
        ...
        boardToArray();
        try {
            Call<ResponseBody> call = dataAPI.getPredict(board[0], board[1], board[2], board[3], board[4], board[5], board[6], board[7], board[8]);
    ...
    private void result(String title) {
        String message = "\nCIRCULOS " + noughtsScore + "\n\nCRUCES " + crossesScore;
        new AlertDialog.Builder(this)
                .setTitle(title)
                .setMessage(message)
                .setPositiveButton(">>Volver a jugar<<", (dialog, which) -> resetBoard())
                .setCancelable(false)
                .show();
    }
    ...
    private void addToBoard(Button button) {
        ...
        if (currentTurn == Turn.NOUGHT) {
            ...
            button.setBackground(getResources().getDrawable(R.drawable.oimage));
            currentTurn = Turn.CROSS;
        } else if (currentTurn == Turn.CROSS) {
            button.setText(CROSS);
            button.setBackground(getResources().getDrawable(R.drawable.ximage));
